...
namespace ASL.PathTracer
{
	class BSPTree : SceneData
	{
		...
		{
			return RayIntersect(ray, epsilon, 0.0, double.MaxValue, ref hit);
		}

		private bool RayIntersect(Ray ray, double epsilon, double min, double max, ref RayCastHit hit)
		{
			var node = m_Root;
			if (node == null)
				return false;
			...
			while (true)
			{
				if (!node.IsLeaf)
				{
					...
					bool nearIndex = dist > 0.0;
					if (denom > double.Epsilon || denom < -double.Epsilon)
					{
						double t = dist / denom;
						if (0.0 <= t && t <= max)
						{
							if (t >= min)
							{
								int nindex = !nearIndex ? 1 : 0;
								nodeStack.Push(node.GetChild(nindex));
								timeStack.Push(max);
								max = t;
							}
							else
					...
					node = node.GetChild(nearIndex ? 1 : 0);
				}
				else
				{
					if (node.triangles != null)
					{
						...
						{
							bool result = node.triangles[i].RayCast(ray, epsilon, ref hit);
							if (result && hit.distance >= min && hit.distance <= max)
								ishit = true;
						}
						if (ishit)
							return true;
			...
			List<Vertex> behindVertices)
		{
			...
			double dist = (plane.d - Vector3.Dot(begin.position, plane.normal)) / Vector3.Dot((end.position - begin.position).normalized, plane.normal);
			dist /= Vector3.Distance(begin.position, end.position);
			//dist = 1.0 - dist;

			if ((beginPt == PointType.InFrontOfPlane || beginPt == PointType.OnPlane) &&
		...
		class BSPTreeNode
		{
			...
			public BSPTreeNode GetChild(int index)
			{
				if (index == 0)
					return front;
				else if (index == 1)
